    ...
    if(["1️⃣", "2️⃣"].includes(emoji) && message.channel.id === channel.id) {
        switch (emoji) {
            case "1️⃣": 
                ...
                message.channel.send(`Le rôle ${membreRole.name} a été ajouté avec succès !`);
                break;
            case "2️⃣": 
                member.roles.add(ModerateurRole);
                message.channel.send(`Le rôle ${ModerateurRole.name} a été ajouté avec succès !`);
                break;
        }
    }
}